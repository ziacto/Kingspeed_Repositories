...
- (void)_XBFilteredCameraViewInit
{
    ...
    //
    _focusView = [[XBCameraFocusView alloc] initWithFrame:CGRectMake(0, 0, 65, 65)];
    [self addSubview:_focusView];
    [_focusView hideAnimated:NO];
}
...
- (void)focusAtPoint:(CGPoint)focusPoint__
{
    self.focusPoint = focusPoint__;
    self.exposurePoint = focusPoint__;
    
    if (self.exposurePointSupported || self.focusPointSupported) {
        _focusView.center = self.exposurePoint;
        [_focusView showAnimated:YES];
    }
}
...
- (void)takeAPhotoWithCompletion:(void (^)(UIImage *,  XBPhotoOrientation))completion
{
    ...
    if (self.flashMode == XBFlashModeOn) {
        UIView *flashView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
    ...
    [self.stillImageOutput captureStillImageAsynchronouslyFromConnection:imageConnection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
        ...
        //======================================================================
        if ([self cameraPosition] == XBCameraPositionFront) {
            filteredImage = [UIImage imageWithCGImage:filteredImage.CGImage
                                                scale:1.0 orientation: UIImageOrientationUpMirrored];
        }
        //======================================================================

        completion(filteredImage, orientation);
        self.rendering = YES;
    }];
}
