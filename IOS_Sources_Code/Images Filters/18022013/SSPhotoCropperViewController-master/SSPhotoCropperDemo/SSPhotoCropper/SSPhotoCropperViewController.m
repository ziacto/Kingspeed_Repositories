...
- (void) loadPhoto
{
    ...
    CGFloat rate = w/h > 320./416. ? h/416. : w/320.;
    
    CGRect imageViewFrame = CGRectMake(0.0f, 0.0f, roundf(w / rate), roundf(h / rate));
    self.scrollView.contentSize = self.scrollView.frame.size;

    UIImageView *iv = [[UIImageView alloc] initWithFrame:imageViewFrame];
    iv.image = self.photo;
    iv.center = CGPointMake(scrollView.frame.size.width/2., scrollView.frame.size.height/2.);
    [self.scrollView addSubview:iv];
    self.imageView = iv;
    [iv release];
    
    scrollView.minimumZoomScale = 1.;
    scrollView.maximumZoomScale = rate > 1. ? rate : 1.;
}
