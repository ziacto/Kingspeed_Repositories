...
@property (nonatomic, strong) NSMutableArray *pointsForDraw;
@property (nonatomic, strong) NSMutableArray *pointsForSelect;
...
- (void)addPointsAt:(NSArray *)points
{
    ...
    self.pointsForDraw = tmp;
    self.pointsForSelect = [NSMutableArray arrayWithArray:tmp];
}
...
- (NSArray *)getPoints
{
    ...
    for (uint i=0; i<self.pointsForDraw.count; i++)
    {
        UIView *v = [self.pointsForDraw objectAtIndex:i];
...
{
    if (self.pointsForDraw.count <= 0) return nil;
...
{
    if (self.pointsForDraw.count <= 0) return;
    ...
    UIView *point1 = [self.pointsForDraw objectAtIndex:0];
    CGContextMoveToPoint(context, point1.frame.origin.x +k_POINT_WIDTH/2, point1.frame.origin.y +k_POINT_WIDTH/2);
    
    for (uint i=1; i<self.pointsForDraw.count; i++)
    {
        UIView *point = [self.pointsForDraw objectAtIndex:i];
...
- (BOOL)pointInside:(CGPoint)point withEvent:(UIEvent *)event{
    if (self.pointsForDraw.count <= 0) return NO;
    
    if ((CGRectContainsPoint(self.bounds, point))) {
        CGPoint locationPoint = point;
        
        for (UIView *pointView in self.pointsForDraw)
        {
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    for (UIView *point in self.pointsForSelect)
    {
        ...
        if ([point pointInside:viewPoint withEvent:event])
        {
            ...
            [self addSubview:point];
            [self.pointsForSelect removeObject:point];
            [self.pointsForSelect insertObject:point atIndex:0];
