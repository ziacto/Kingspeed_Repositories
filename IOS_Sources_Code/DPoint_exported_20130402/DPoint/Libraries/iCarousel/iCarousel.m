...
#define DECELERATION_MULTIPLIER 70.0f	//30.0f	// 数値が大きいほど止まりやすい.
...
- (CGFloat)ratioStepClip:(CGFloat)orgRatio threshold:(CGFloat)threshold
{
	if( orgRatio < threshold ){
		float	ratioOuter	= 1.0f - (orgRatio / threshold);	// 外側:1.0 .. 内側:0.0f
		return ratioOuter;
	}
	else{
		float	ratioInner	= ((orgRatio-threshold) / (1.0f-threshold));	// 内側:1.0 .. 内側:0.0f
		return ratioInner;
	}
}
...
- (CGFloat)alphaForItemWithOffset:(CGFloat)offset
{
	...
	switch (_type)
	{
		case iCarouselTypeDPointMenuB:
		case iCarouselTypeDPointMenuD:
		{
			if ( fabsf(offset) <= DPMENU_THRESHOLD_CENTER)
			{	// 中央より.
				// offset .. 上:-DPMENU_THRESHOLD_CENTER .. 中央:0.0 .. 下:+DPMENU_THRESHOLD_CENTER
				CGFloat	offsetNormalized	= fabsf(offset)/DPMENU_THRESHOLD_CENTER;	// 中央:0.0 .. 上下:+1.0
				CGFloat	ratioCenter	= 0.5f + (0.5f*cosf(offsetNormalized*M_PI));	// 中央:1.0 .. 周辺:0.0
				CGFloat	ratioCenterClipped	= [self clipRatioFrom:(1.0f-offsetNormalized) thTop:0.7f];
				
				if( ratioCenterClipped > 0.0f && ratioCenterClipped < 1.0f ){
					return [self ratioStepClip:ratioCenterClipped threshold:DPMENU_THRESHOLD_ALPHA];
//					return 0.2f;
				}
				else{
					return 1.0f;
				...
				/*
				offsetX	= offsetXMin + offsetXDelta * ratioCenterClipped;
				if( offset < 0.0f ){
					offsetY	= -offsetYDelta*(1.0f-ratioCenter);
				}
				else{
					offsetY	= offsetYDelta*(1.0f-ratioCenter);
				}
				offsetZ	= offsetZFar + offsetZDelta * ratioCenterClipped;
				
				CGFloat	invRatioCenter	= 1.0f-ratioCenter;	// 中央:0.0 .. 周辺:1.0
				spacing	*= invRatioCenter;
				 */
			}
		}
		default:
...
/**
 *	@param	orgRatio	元の値(0.0 .. 1.0).
 */
- (float)clipRatioFrom:(CGFloat)orgRatio thTop:(CGFloat)thTop
{
	if( orgRatio > thTop ){
		return 1.0f;
	}
	else{
		float	localRatio	= (orgRatio/thTop);	// (0.0..thTop) → (0.0..1.0)
		return (0.5f + 0.5f*cosf((1.0f-localRatio)*M_PI));
	}
}
...
- (CATransform3D)transformForItemView:(UIView *)view withOffset:(CGFloat)offset
{
	...
	switch (_type)
	{
		...
		case iCarouselTypeDPointMenuA:
		case iCarouselTypeDPointMenuB:
		case iCarouselTypeDPointMenuC:
		case iCarouselTypeDPointMenuD:
		{
			//TODO: <ポイントアプリ用演出>
			// ----- ここを調整する ----- //
...
//			NSLog(@"height[%d]", (int)viewHeight);	// 416px:3.5inch, 504px:4inch
			float	spacingDelta	= (viewHeight - 416.0f)*0.001f;
			CGFloat spacing = [self valueForOption:iCarouselOptionSpacing withDefault:0.8f+spacingDelta];
			
			CGFloat arc = [self valueForOption:iCarouselOptionArc withDefault:M_PI * 2.0f];
			CGFloat angle = [self valueForOption:iCarouselOptionAngle withDefault:arc / count];
			
			// 奥行き方向の調整パラメータ.
			const CGFloat	offsetZFar	= -1200.0f;
			const CGFloat	offsetZNear	= 50.0f;
			const CGFloat	offsetZDelta	= offsetZNear - offsetZFar;
			CGFloat	offsetZ	= offsetZFar;
			// X方向の調整パラメータ.
			const CGFloat	offsetXMin	= -280.0f;
			const CGFloat	offsetXMax	= 0.0f;
			const CGFloat	offsetXDelta	= offsetXMax - offsetXMin;
			CGFloat	offsetX	= offsetXMin;
			// Y方向の調整パラメータ.
			const CGFloat	offsetYDelta	= 100.0f;
			CGFloat	offsetY	= 0.0f;
			
			// offset .. 中央:0.0 .. 上:-value, 下:+value
			if (offset <= -DPMENU_THRESHOLD_CENTER)
			{	// 上側.
				offsetY	= -offsetYDelta;
			}
			else if (offset <= DPMENU_THRESHOLD_CENTER)
			{
				...
				if( _type == iCarouselTypeDPointMenuC || _type == iCarouselTypeDPointMenuD )
				{
					offsetNormalized	= offset/DPMENU_THRESHOLD_CENTER;	// 中央:0.0 .. 上下:+1.0
					CGFloat	clampedOffset = fmaxf(-1.0f, fminf(1.0f, offset));
					if (_toggle >= 0.0f){
						if (offset <= -0.5f){
							clampedOffset = -1.0f;
					...
					ratioCenter	= 0.5f + (0.5f*cosf(fabsf(offset)*M_PI));	// 中央:1.0 .. 周辺:0.0
					ratioCenterClipped	= [self clipRatioFrom:(1.0f-fabsf(offset)) thTop:0.7f];
				}
				...
				if( _type == iCarouselTypeDPointMenuA || _type == iCarouselTypeDPointMenuC ){
					offsetX	= offsetXMin + offsetXDelta * ratioCenterClipped;
					offsetZ	= offsetZFar + offsetZDelta * ratioCenterClipped;
				}
				else{
					// for iCarouselTypeDPointMenuB
					if( ratioCenterClipped < DPMENU_THRESHOLD_ALPHA ){
						offsetX	= offsetXMin;
						offsetZ	= offsetZFar;
					}
					else{
			...
			transform = CATransform3DTranslate(transform, offsetX, offsetY, offsetZ);
			// +Y 下, -Y 上.
			// +Z 手前, -Z 奥.
