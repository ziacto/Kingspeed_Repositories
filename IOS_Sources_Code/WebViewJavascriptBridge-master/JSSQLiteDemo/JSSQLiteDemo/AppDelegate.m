...
- (void)createEditableCopyOfDatabaseIfNeeded {
    ...
    NSString* dbPath = [docPath stringByAppendingPathComponent:@"JSSQLiteDemo.sqlite"];
    ...
    if(![fm fileExistsAtPath:dbPath])
    {
        ...
        NSString* dbTemplatePath = [[NSBundle mainBundle] pathForResource:@"JSSQLiteDemo" ofType:@"sqlite"];
        NSError* error = nil;
        [fm copyItemAtPath:dbTemplatePath toPath:dbPath error:&error];
        if(error){
            NSLog(@"can't copy db template: %@.", error.userInfo);
            assert(false);
        }
    }
}
