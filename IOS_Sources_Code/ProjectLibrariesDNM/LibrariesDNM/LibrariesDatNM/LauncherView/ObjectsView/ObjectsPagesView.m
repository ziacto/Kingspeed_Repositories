...
static const int maxPageCount = 1000;
...
static const CGFloat portraitItemYStart = 20;
static const CGFloat portraitXPadding = 0;
static const CGFloat portraitYPadding = 10;
...
/* iPad */
static const int iPadMaxItemsPageCount = 9;
...
-(BOOL)itemMovable:(OneObjectView *)itemToSearch;
-(void)savePages;
...
@property (nonatomic, retain) OneObjectView *draggingItem;
@end

@implementation ObjectsPagesView
...
- (id)initWithFrame:(CGRect)frame 
{
    if ((self = [super initWithFrame:frame])) 
	{
		...
		[self setPagesScrollView:[[ObjectsScrollView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height - pControllHeight)]];
		...
		[self setPageControl:[[ObjectsPageControl alloc] initWithFrame:CGRectMake(0, frame.size.height - 44 - 48 - pControllHeight, frame.size.width, pControllHeight)]]; //if starts landscape this will break...
...
//	self.pagesScrollView.contentOffset = CGPointMake(self.pageControl.currentPage * self.pagesScrollView.frame.size.width, 0);
    [self.pagesScrollView setContentOffset:CGPointMake(self.pageControl.currentPage * self.pagesScrollView.frame.size.width, 0) animated:YES];
}
...
//	self.pageControl.frame = CGRectMake(0, self.frame.size.height - pControllHeight, self.frame.size.width, pControllHeight);
    self.pageControl.frame = CGRectMake(0, self.frame.size.height - 44 - 48 - pControllHeight, self.frame.size.width, pControllHeight);
...
-(void)layoutItems
{
	...
	for (NSMutableArray *page in self.pages)
	{
        ...
        CGFloat y = minY;
		int itemsCount = 1;
		for (OneObjectView *item in page)
		{
			if(itemsAdded) 
			{
...
-(void)organizePages
{
	...
	for(NSMutableArray *page in self.pages)
	{
		...
		if(page.count > imaxItemsPageCount)
		{
			...
			if(nextPage)
			{
				OneObjectView *moveItem = [page lastObject];
...
-(void)itemTouchedUpInside:(OneObjectView *)item
{
	if(editing)
	{
		dragging = NO;
        ...
        [self performSelector:@selector(endEditing) withObject:nil afterDelay:0.1];
	}
	else 
	{
...
-(void)itemTouchedUpOutside:(OneObjectView *)item
{
	[self.movePagesTimer invalidate];
	self.movePagesTimer = nil;
	[self.itemHoldTimer invalidate];
...
-(void)itemTouchedDown:(OneObjectView *)item
{
	if (editing)
	{
		if (!self.draggingItem && [self itemMovable:item])
		{
			self.draggingItem = (OneObjectView*)item; 
...
-(void)itemTouchCancelled:(OneObjectView *)item
{
    if (editing) {
...
-(void)itemHoldTimer:(NSTimer *)timer
{
    ...
    OneObjectView *heldItem = (OneObjectView*)timer.userInfo;
...
-(void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
	...
	if(dragging) 
	{
		for (UITouch* touch in touches) 
		{
			...
			NSInteger oldPage = sItemLocation.page;
			NSInteger oldIndex = sItemLocation.sindex;
            ...
            NSMutableArray *itemPage = [self.pages objectAtIndex:oldPage];
            if(currentPageIndex != oldPage && dragIndex >= [itemPage count])
            {
                dragIndex = 0;
            }
            
			if(oldIndex != dragIndex || (dragIndex == 0 && [itemPage count] == 1 && currentPageIndex != oldPage))// || currentPageIndex != oldPage)
			{
                if (dragIndex < [itemPage count])
                {
                    OneObjectView *itemToMove = [itemPage objectAtIndex:dragIndex];
                    if ([self itemMovable:itemToMove]) {
                        [itemPage removeObjectAtIndex:oldIndex];
                        ...
                        else 
                        {
                            [currentPage insertObject:self.draggingItem atIndex:dragIndex];
                            [self organizePages];
                            [UIView animateWithDuration:TIME_ANIMATION_POSITION 
                                             animations:^{
                                                 [self layoutItems]; 
                                             }];
                        }
			...
			//Moving Pages
			if(location.x + self.pagesScrollView.contentOffset.x < self.pagesScrollView.contentOffset.x + AREA_HOLD_TO_MOVE_PAGES)
			{
				if(currentPageIndex > 0)
				{
					if(!self.movePagesTimer)
						[self setMovePagesTimer:[NSTimer scheduledTimerWithTimeInterval:TIME_HOLD_TO_MOVE_PAGES
                                                                                 target:self 
			...
			else if(location.x + self.pagesScrollView.contentOffset.x > self.pagesScrollView.contentOffset.x + self.pagesScrollView.frame.size.width - AREA_HOLD_TO_MOVE_PAGES)
			{
				if(!self.movePagesTimer)
					[self setMovePagesTimer:[NSTimer scheduledTimerWithTimeInterval:TIME_HOLD_TO_MOVE_PAGES
                                                                             target:self 
...
-(void)movePagesTimer:(NSTimer*)timer
{
	...
	if([(NSString*)timer.userInfo isEqualToString:NEXT_PAGE])
	{	
	...
	else if([(NSString*)timer.userInfo isEqualToString:PREV_PAGE])
	{
		NSInteger currentPageIndex = floor(self.pagesScrollView.contentOffset.x/self.pagesScrollView.frame.size.width);
		self.pageControl.currentPage = --currentPageIndex;
		CGFloat newX = self.pagesScrollView.contentOffset.x - self.pagesScrollView.frame.size.width;
...
-(void)endEditing
{
	...
	for (int i = 0; i < self.pages.count; ++i) 
	{
		...
		{
			for (OneObjectView* item in itemPage) 
				item.transform = CGAffineTransformIdentity;
		}
	}
...
{
	OneObjectView *ditem = (OneObjectView*)item;
	
	for (NSMutableArray *page in self.pages)
	{
		int i = 0;
		for (OneObjectView *aitem in page)
		{
			if(aitem == ditem)
			{
...
#pragma mark SAVE PAGES AFTER EDIT
-(void)savePages
{
...
//            [itemToSave setObject:item.iPadImage forKey:@"iPadImage"];
            ...
            [itemToSave setObject:[NSNumber numberWithInt:2] forKey:@"ObjectsPagesViewItemVersion"];
...
//	[self saveToUserDefaults:pagesToSave key:@"ObjectsPagesView"];
//    [self saveToUserDefaults:[NSNumber numberWithInt:numberOfImmovableItems] key:@"ObjectsPagesViewImmovable"];
}
